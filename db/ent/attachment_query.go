...
func (aq *AttachmentQuery) QueryApplications() *ApplicationQuery {
	query := &ApplicationQuery{config: aq.config}
...
func (aq *AttachmentQuery) Clone() *AttachmentQuery {
	...
	return &AttachmentQuery{
		...
		withApplications: aq.withApplications.Clone(),
...
func (aq *AttachmentQuery) sqlAll(ctx context.Context) ([]*Attachment, error) {
	...
	if query := aq.withApplications; query != nil {
	...
	if query := aq.withTickets; query != nil {
		...
		nodeids := make(map[int32][]*Attachment)
		for i := range nodes {
