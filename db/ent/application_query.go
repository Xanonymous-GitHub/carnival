...
type ApplicationQuery struct {
	...
	withAssignmentHistories *ApplicationAssignmentHistoryQuery
	withStatusHistories     *ApplicationStatusHistoryQuery
...
func (aq *ApplicationQuery) QueryTickets() *TicketQuery {
	query := &TicketQuery{config: aq.config}
...
func (aq *ApplicationQuery) QueryAssignmentHistories() *ApplicationAssignmentHistoryQuery {
	query := &ApplicationAssignmentHistoryQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ApplicationQuery) QueryStatusHistories() *ApplicationStatusHistoryQuery {
	query := &ApplicationStatusHistoryQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ApplicationQuery) Clone() *ApplicationQuery {
	...
	return &ApplicationQuery{
		...
		withTickets:             aq.withTickets.Clone(),
		withAssignmentHistories: aq.withAssignmentHistories.Clone(),
		withStatusHistories:     aq.withStatusHistories.Clone(),
...
func (aq *ApplicationQuery) WithTickets(opts ...func(*TicketQuery)) *ApplicationQuery {
	...
	aq.withTickets = query
	return aq
}

// WithAssignmentHistories tells the query-builder to eager-load the nodes that are connected to
// the "assignment_histories" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ApplicationQuery) WithAssignmentHistories(opts ...func(*ApplicationAssignmentHistoryQuery)) *ApplicationQuery {
	query := &ApplicationAssignmentHistoryQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withAssignmentHistories = query
	return aq
}

// WithStatusHistories tells the query-builder to eager-load the nodes that are connected to
// the "status_histories" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ApplicationQuery) WithStatusHistories(opts ...func(*ApplicationStatusHistoryQuery)) *ApplicationQuery {
	query := &ApplicationStatusHistoryQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withStatusHistories = query
	return aq
}
...
func (aq *ApplicationQuery) sqlAll(ctx context.Context) ([]*Application, error) {
	var (
		...
		loadedTypes = [4]bool{
			aq.withTickets != nil,
			aq.withAssignmentHistories != nil,
			aq.withStatusHistories != nil,
	...
	if query := aq.withTickets; query != nil {
	...
	if query := aq.withAssignmentHistories; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Application)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
	...
	if query := aq.withStatusHistories; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Application)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
