...
func BotActiveStatusValidator(bas schema.BotActiveStatus) error {
	switch bas {
	case "active", "suspend", "delete":
		return nil
	default:
...
func StoreTypeValidator(st schema.StoreType) error {
	switch st {
	case "online_store", "physical_store":
		return nil
	default:
...
func ApplicationStatusValidator(as schema.ApplicationStatus) error {
	switch as {
	case "wip", "reviewing", "verified", "rejected", "waiting", "replied", "revoked", "canceling":
		return nil
	default:
